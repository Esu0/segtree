...
/// ランダムなセグメント木を生成する
fn get_segtree(n: usize) -> SegTree<MinQuery<i64>> {
    thread_local! {
        static RNG: RefCell<StdRng> = RefCell::new(StdRng::seed_from_u64(100));
    }
    RNG.with(|rng| {
        let mut rng = rng.borrow_mut();
        iter::repeat_with(|| MinQuery(rng.gen_range(-1_000_000_000i64..=1_000_000_000)))
            .take(n)
            .collect::<SegTree<_>>()
    })
}
...
fn bench_segtree(c: &mut Criterion) {
    ...
    c.bench_function("segtree-update", |b| {
        ...
        b.iter(|| {
            seg.update(
                ...
                black_box(MinQuery(data_iter.next().unwrap())),
            );
        })
    });
    ...
    c.bench_function("segtree-query-update", |b| {
        ...
        b.iter(|| {
            ...
            let d = black_box(MinQuery(data_iter.next().unwrap()));
    ...
    c.bench_function("segtree-partition-point", |b| {
        ...
        b.iter(|| {
            ...
            black_box(seg.partition_point(0, |x| x.0 > d));
        })
    });
    ...
    c.bench_function("segtree-partition-point-range", |b| {
        ...
        b.iter(|| {
            let i = black_box(index_iter.next().unwrap());
            let d = black_box(data_iter.next().unwrap());
            black_box(seg.partition_point(i, |x| x.0 > d));
        })
    });
}
